const starterPosts = [
  {
    id: "minatoloader-paper-note",
    title: "MinatoLoader 论文解析：用样本感知调度缓解数据预处理瓶颈",
    category: "论文",
    tags: ["AI Infra", "ML Systems", "DataLoader", "PyTorch", "EuroSys", "论文精读"],
    date: "2026-05-05",
    excerpt: "MinatoLoader 通过识别快慢样本、后台处理慢样本和动态调节 CPU workers，将数据预处理从 GPU 训练瓶颈中解耦出来。",
    content: `## 🌟 引言
...
function loadPosts() {
  ...
  try {
    ...
    const savedIds = new Set(parsed.map((post) => post.id));
    const missingStarterPosts = starterPosts.filter((post) => !savedIds.has(post.id));
    const mergedPosts = [...missingStarterPosts, ...parsed];

    if (missingStarterPosts.length) {
      localStorage.setItem(storageKey, JSON.stringify(mergedPosts));
    }
