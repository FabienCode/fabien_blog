...
- **轻量深度适配**：复制预训练 DiT 最后若干层作为 depth branch，让深度分支读取主分支特征，但不反向干扰 RGB/action 主干。
- **Asynchronous Noise Sampling**：动作分支先用少量 step 去噪并立即执行，视频分支继续完成更多 step，用于高保真世界生成。
- **训练-推理分布对齐**：训练时不独立采样视频和动作噪声，而是约束 `t_O >= t_a` 的联合分布，使训练覆盖推理时“动作先干净、视频继续去噪”的状态。

> 关键判断：这篇论文最有价值的地方，是把“异步推理”变成了一个训练分布设计问题，而不是只在 inference 阶段做工程加速。

### 实验结论

- **策略成功率**：RoboCasa 平均成功率达到 79.2%，比最强 baseline Cosmos Policy 的 67.1% 高 12.1 个百分点。
- **RoboTwin 2.0**：Clean / Randomized 分别达到 89.8% / 90.7%，超过 Motus 的 88.7% / 87.0%。
- **4D 重建**：RoboCasa 上 PSNR、SSIM、LPIPS、AbsRel、delta1、Chamfer Distance 均为最佳；CD 从 DreamZero+DA3 的 0.0680 降到 0.0049。
- **实时部署**：真实双臂耳机收纳任务中，8 step 异步推理在 RTX 5090 D 上约 300 ms/action chunk，并结合 RTC 实现真实机器人闭环执行。

> 关键判断：X-WAM 的提升不只来自更大的视频先验，也来自显式空间监督和异步噪声分布对齐带来的系统性收益。
